# Homebrew Formula for Mumei — auto-generated by release workflow
class Mumei < Formula
  desc "Mathematical Proof-Driven Programming Language — formally verified with Z3"
  homepage "https://github.com/mumei-lang/mumei"
  license "MIT"
  version "0.5.7"

  on_macos do
    if Hardware::CPU.arm?
      url "https://github.com/mumei-lang/mumei/releases/download/v0.5.7/mumei-aarch64-apple-darwin.tar.gz"
      sha256 "1a961dfc9f23802a90e04d175992367d980f11ca3e72b798552fa9e7f3e80e32"
    else
      url "https://github.com/mumei-lang/mumei/releases/download/v0.5.7/mumei-x86_64-apple-darwin.tar.gz"
      sha256 "9f4e46ef820d6acf55535deede8acec2e43a849295824130657d6a9b1fc69636"
    end
  end

  on_linux do
    if Hardware::CPU.arm?
      url "https://github.com/mumei-lang/mumei/releases/download/v0.5.7/mumei-aarch64-unknown-linux-gnu.tar.gz"
      sha256 "57c59813e42c6fe7f2a831d034b347e1ef525a1ebba64552b35492d30d6950f4"
    else
      url "https://github.com/mumei-lang/mumei/releases/download/v0.5.7/mumei-x86_64-unknown-linux-gnu.tar.gz"
      sha256 "3d4ccc9aae584afc1c9d7f329bc3664a3042ace3eabdb291c672d62f1d303e8f"
    end
  end

  depends_on "z3"
  depends_on "llvm@17"

  def install
    bin.install "mumei"
    (share/"mumei/std").install Dir["std/*"]

    # SI-5 Phase 3-C: install the std/ proof-certificate bundle when it
    # ships with the release tarball so downstream projects can verify
    # imports against a trusted, versioned certificate set.
    has_proof_bundle = File.exist?("std-proof-bundle.json")
    if has_proof_bundle
      (share/"mumei").install "std-proof-bundle.json"
    end

    env_script = "export MUMEI_STD_PATH=\"#{share}/mumei/std\"\n"
    if has_proof_bundle
      env_script += "export MUMEI_PROOF_BUNDLE=\"#{share}/mumei/std-proof-bundle.json\"\n"
    end
    (etc/"mumei").mkpath
    (etc/"mumei/env.sh").write env_script
  end

  def caveats
    s = <<~EOS
      The Mumei standard library has been installed to:
        #{share}/mumei/std

    EOS
    if (share/"mumei/std-proof-bundle.json").exist?
      s += <<~EOS
        The std/ proof-certificate bundle (SI-5 Phase 3-C) is at:
          #{share}/mumei/std-proof-bundle.json

      EOS
    end
    s += <<~EOS
      To use it, add the following to your shell profile:
        export MUMEI_STD_PATH="#{share}/mumei/std"
    EOS
    if (share/"mumei/std-proof-bundle.json").exist?
      s += "  export MUMEI_PROOF_BUNDLE=\"#{share}/mumei/std-proof-bundle.json\"\n"
    end
    s += <<~EOS

      Or source the environment file:
        source #{etc}/mumei/env.sh
    EOS
    s
  end

  test do
    assert_match version.to_s, shell_output("#{bin}/mumei --version")
  end
end
